$(document).ready(function () {
    ...
    let infoContactModal = $("#contact-media")
    ...
    if (urlParams.has('TYPE')) {
        ...
        title.text('Suscribirse como socio')
        isLabAdmin = true;
  ...
  /*  $("#btn-confirm-email-validation").click(function () {
        ...
        $.ajax({
            ...
            if (response.Result === "OK") {
                ...
                Swal.fire(response.Message,  'Intente de nuevo', 'info')
                EmailNotConfirmed()
            }
        }
        ).fail(function () {
            Swal.fire('Código incorrecto ', 'Intente de nuevo','info')
  ...
  /*  function EmailConfirmed() {
    ...
    function GenerateOTP() {
        ...
        var method = smsMFAInput.prop('checked') ? 'SMS' : 'EMAIL';
        var contact = smsMFAInput.prop('checked') ? phoneInput.val() : emailInput.val();
        var msg = method == 'SMS' ? 'al telefono +(506)' + ' ' + contact : "al correo electrónico"+' '+contact
        ...
        $.ajax({
            ...
            if (response.Result === "OK") {
                ...
                infoContactModal.text(msg)
            }
            else {
                Swal.fire('No se pudo generar el código de seguridad', 'Por favor intente de nuevo', 'Error');
            }
        }
        ).fail(function () {
            Swal.fire('No se pudo generar el código de seguridad', 'Por favor intente de nuevo', 'Error');
    ...
    $("#btn-validateOTP").click(function () {
        let service = "/api/User/ProcessOTPValidation?otp=" + $("#txtOneTimePassword").val();
        let url_API = GetUrlApiService(service);
        ...
        $.ajax({
            ...
            Swal.fire('Error', 'No se pudo procesar el código de seguridad', 'Error')
        });
    });
    ...
    function SubmitRegistration() {
        ...
        newUser.Estatus = 1;
        newUser.DefectRole = isLabAdmin==true ? 2 : 0;
        ...
        $.ajax({
            ...
            Swal.fire('Lo sentimos', 'Ocurrió un error inesperado, por favor intente de nuevo', 'warning')
        });
